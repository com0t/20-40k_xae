...
export default {
    ...
    methods: {
      ...
      getAppointments (first = false) {
        ...
        this.$http.get(`${this.$root.getAjaxUrl}/appointments`, {
          ...
          .then(response => {
            ...
            Object.keys(response.data.data.appointments).forEach((dateKey) => {
      ...
      getPackagePaymentStatus (app) {
        let paidCount = app.filter(b => b.booking.payments && b.booking.payments[0].status === 'paid').length
        if (paidCount === app.length) return 'paid'
        if (paidCount === 0) {
          if (!app.find(b => b.booking.payments && b.booking.payments[0].status === 'partiallyPaid')) return 'pending'
        }
