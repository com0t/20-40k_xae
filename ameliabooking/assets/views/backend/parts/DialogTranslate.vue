<template>
  <div class="am-dialog-translate">
    ...
    <div class="am-dialog-scrollable" :style="dialogStyle">
      ...
      <div v-for="(translation, key) in oldTranslations" :key="key" :class="{'am-lite-container-disabled': $root.isLite}">
        <el-row :gutter="28" type="flex" v-if="usedLanguages && usedLanguages.indexOf(key) !== -1">
          ...
          <!-- Translation -->
          <el-col :xs="8" :sm="14" v-if="type === 'name' && tab !== 'cf'" style="padding-right: 0">
      ...
      <div v-for="(translation, index) in newTranslations" :key="index" :class="{'am-lite-container-disabled': $root.isLite}">
        <el-row :gutter="28" type="flex" style="margin-right: -15px">
          ...
          <el-col :xs="14" :sm="18" v-if="type === 'name' && tab !== 'cf'" style="padding-right: 0">
    ...
    <!-- Dialog Footer -->
    <div class="am-dialog-footer" v-if="tab === 'category' || tab === 'cf' || tab === 'roles'">
...
export default {
  ...
  mounted () {
    ...
    if (this.type === 'description' || this.tab === 'cf' || this.type === 'url') {
  ...
  computed: {
    dialogStyle () {
      if (this.tab !== 'cf' && this.tab !== 'category' && this.tab !== 'roles') {
  ...
  methods: {
    saveDialog () {
      ...
      this.$emit('saveDialogTranslate', JSON.stringify(this.translations), this.newLanguages, this.tab, this.options)
    },
    ...
    getLanguageLabel (lang, cut) {
      if (cut && this.type === 'name' && this.tab !== 'cf') {
    ...
    goBack () {
      if (this.tab === 'cf' || this.tab === 'category') {
        this.closeDialog()
      } else {
        this.saveDialog()
      }
