  ...
  export default {
    ...
    methods: {
      ...
      getCapacity (appointment) {
        let employee = this.getProviderById(appointment.providerId)
        let service = employee.serviceList.find(s => s.id === appointment.serviceId)
        let numOfBookings = 0
      ...
      checkedAppointmentsCount () {
        let cnt = 0
        Object.keys(this.appointmentsDay).forEach((dateKey) => {
