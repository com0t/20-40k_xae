              ...
              <el-row :gutter="16" :class="{'am-lite-disabled': ($root.isLite && createNewContent)}">
                ...
                <el-col :span="!createNewContent ? 12 : 24">
                  ...
                  <div class="align-right" v-if="notification.sendTo === 'customer'">
                    ...
                    <el-select class="select-languages" :placeholder="$root.labels.language" v-model="selectedLanguage" clearable filterable @change="textMode = false" v-popover:languagesPop :disabled="$root.isLite">
    ...
    <transition name="slide">
      ...
      >
        <dialog-combined-placeholder
          ...
          :nameHtml="getPlaceholdersSettingsName + 'Html'"
          :customFields="customFields"
          :userTypeTab="userTypeTab"
          :entity="entity"
          :type="type"
    ...
    methods: {
      ...
      employeeNotifications (entity) {
        if (entity === 'provider_other_notifications' && this.type === 'email') {
          return this.notifications.filter(
            notification => notification.type === 'email' && notification.sendTo === 'provider' && ['provider_panel_access', 'provider_panel_recovery'].indexOf(notification.name) !== -1
          )
        } else {
          return this.notifications.filter(
