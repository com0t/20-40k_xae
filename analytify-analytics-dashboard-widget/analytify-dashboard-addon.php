	...
	class Analytify_Dashboard_Addon {
		...
		*/
		public function wpa_general_dashboard_area( $var, $dashboard_id ) {
			...
			if ( isset( $acces_token ) && ! empty( $acces_token ) && get_option( 'pa_google_token' ) ) {
				...
				$_differ = get_option( 'analytify_widget_date_differ' );
				...
				if ( isset( $_POST['startDate'] ) && ! empty( $_POST['startDate'] ) && isset( $_POST['endDate'] ) && ! empty( $_POST['endDate'] ) ) {
					$start_date	= sanitize_text_field( wp_unslash( $_POST['startDate'] ) );
					$end_date	= sanitize_text_field( wp_unslash( $_POST['endDate'] ) );
				}
				...
				if ( $GLOBALS['WP_ANALYTIFY']->settings->get_option( 'profile_for_dashboard', 'wp-analytify-profile', '' ) != '' ) {
					...
					<div class="analytify_wraper">
						<div class="analytify-activation-card-header">
							<img src="<?php echo plugins_url( 'assets/images/logo.svg', __FILE__ ); ?>">
						</div>
		...
		*/
		function exception_message( $message ) {
			...
			<div class="analytify-activation-cards">
				<div class="analytify-activation-card-header">
						<img src="' . plugins_url( 'assets/images/logo.svg', __FILE__ ) . '">
				</div>
		...
		*/
		public static function analytify_general_stats() {
			...
			if ( isset( $_POST['dateDiff'] ) && ! empty( $_POST['dateDiff'] ) ) {
				update_option( 'analytify_widget_date_differ', $_POST['dateDiff'] );
			}
